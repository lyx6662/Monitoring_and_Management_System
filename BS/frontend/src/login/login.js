...
function Login() {
  ...
  const fetchCaptcha = async () => {
    try {
      const response = await axios.get('http://8.149.241.140:5000/api/captcha');
      setCaptcha({
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (showCaptcha) {
        try {
          const verifyRes = await axios.post('http://8.149.241.140:5000/api/verify-captcha', {
      ...
      const response = await axios.post(
        'http://8.149.241.140:5000/api/auth/login',
        { account, password }
      );
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'http://8.149.241.140:5000/api/auth/register',
        {
          username,
          account,
          password,
