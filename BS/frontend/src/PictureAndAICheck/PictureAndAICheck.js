...
const PictureAndAICheck = () => {
  ...
  const loadValidImages = async () => {
    try {
      ...
      const res = await fetch('http://116.62.54.160:5000/api/oss/files?directory=check_1/', {
        headers: {
  ...
  const loadInvalidImages = async () => {
    try {
      ...
      const res = await fetch('http://116.62.54.160:5000/api/oss/files?directory=check_2/', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
