...
const PictureAndAICheck = () => {
  ...
  const loadFiles = async () => {
    try {
      ...
      const res = await fetch('http://116.62.54.160:5000/api/oss/files', {
        headers: {
  ...
  const loadAllCheckImages = async () => {
    try {
      ...
      // 加载待检查图片
      const checkRes = await fetch('http://116.62.54.160:5000/api/oss/files?directory=check/', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      // 加载检查正确图片
      const validRes = await fetch('http://116.62.54.160:5000/api/oss/files?directory=check_1/', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      // 加载检查错误图片
      const invalidRes = await fetch('http://116.62.54.160:5000/api/oss/files?directory=check_2/', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const checkWithAI = async (imageUrl, fileName) => {
    try {
      ...
      const response = await axios.post(
        'http://116.62.54.160:5000/api/ai-vision-check',
